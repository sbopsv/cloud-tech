...
function isMdfileThan1YearAgo(date) {
  const mdfileUploadDate = new Date(date);

  const currentDate = new Date();

  const dateDiff = currentDate - mdfileUploadDate;

  const yearDiff = Math.floor(dateDiff / 1000 / 60 / 60 / 24 / 30 / 12);

  let outputDateDiffString = "";

  if (yearDiff !== 0) {
    outputDateDiffString += `この記事は最終更新日から${yearDiff}年以上が経過しています。`;
    return <Warning>{outputDateDiffString}</Warning>;
  }
  return <Normal> </Normal>;
}

function isMenuPage(thumbnail, date) {
  if(thumbnail.length > 0) {
      return(
        <div className="updateWrapper">
        <Badge>作成日:{date}</Badge>  
        {isMdfileThan1YearAgo(date)} 
        </div>        
      )
  }else{
    return(
...
//    canonicalUrl = config.gatsby.pathPrefix !== '/' ? canonicalUrl + config.gatsby.pathPrefix : canonicalUrl;
    ...
    return (
      <Layout {...this.props}>
        ...
        <BreadCrumbHeader>
          <SubHeader location={location} title={mdx.frontmatter.metaTitle}/> 
          <EditGithubBtn cNmae="mobileAlign" docsLocation={docsLocation} parentRelativePath={mdx.parent.relativePath} />
        </BreadCrumbHeader>
        <div className="titleWrapper">
          <h1 className="title">{mdx.frontmatter.metaTitle}</h1>
        </div>
        {isMenuPage(mdx.frontmatter.thumbnail, mdx.frontmatter.date)}
        <div className="mainWrapper">
          <MDXRenderer>{mdx.body}</MDXRenderer>
          {
          <FloatingSubscribeNewsletter title={mdx.frontmatter.metaTitle} canonicalUrl={canonicalUrl} location={location} />
          }
        </div>
        <HelpfulGithubWrapper>
          <div className="helpfulWrapper">
            <div className="desc">このページは参考になりましたか？</div>
            <div className="iconWrapper">
              <div className={"satisfied" + ((isSatisfied) ? " active" : "")} onClick={()=> this.handlePageHelpfulResponse(true)}>
                ...
                </svg>
                {
                  showTooltip && isSatisfied ? (
                    <div className="toolTip">フィードバックをありがとうございました：）</div>
                  ) : null
                }
              </div>
              <div className={"dissatisfied" + ((isSatisfied === false) ? " active" : "")} onClick={()=> this.handlePageHelpfulResponse(false)}>
                ...
                </svg>
                {
                  showTooltip && isSatisfied !== null && !isSatisfied ? (
                    <div className="toolTip">フィードバックをありがとうございました。改善するように頑張ります。</div>
                  ) : null
                }
              </div>
            </div>
